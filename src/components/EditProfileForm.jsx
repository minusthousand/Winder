...
function makeSlots(images) {
  return (images || []).map((url) => ({ id: ++slotId, url, uploading: false, error: null }));
}

export default function EditProfileForm({ profile, onClose, onSave, onDelete }) {
  ...
  const [likesYou, setLikesYou] = useState(profile.likesYou ?? false);
  const [slots, setSlots] = useState(() => makeSlots(profile.images));
  ...
  const submit = async () => {
    ...
    try {
      await onSave({
        ...profile,
        name: form.name.trim(),
        age: parseInt(form.age, 10) || profile.age,
        likesYou,
        job: form.job.trim() || profile.job,
  ...
  return (
    <div onClick={dismiss} className={`absolute inset-0 z-[55] flex items-end ${closing ? 'animate-mwFadeOut' : 'animate-mwFade'}`}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between mb-[18px]">
          <div style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 700, fontSize: 21, color: '#3a2740' }}>Edit profile</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
          {slots.map((slot) => (
            <div key={slot.id} style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 12, overflow: 'hidden', background: '#f6eef1' }}>
              {slot.url && <img src={slot.url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
              {slot.uploading && (
        ...
        <button onClick={submit} disabled={submitting || anyUploading}
          ...
          {anyUploading ? 'Uploading…' : submitting ? 'Saving…' : 'Save changes'}
        </button>

        {/* Delete */}
        {!confirmDelete ? (
          <button onClick={() => setConfirmDelete(true)} disabled={submitting}
            className="w-full mt-3 py-[13px] border-none rounded-[15px] font-bold text-[14px] cursor-pointer"
            style={{ background: '#fff5f6', color: '#FF4F6B' }}>
            Delete profile
          </button>
        ) : (
          <div style={{ marginTop: 12, padding: '14px 16px', background: '#fff5f6', borderRadius: 15 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600, color: '#3a2740', textAlign: 'center', marginBottom: 12 }}>
              Remove {profile.name} from the deck?
            </div>
            <div className="flex gap-[10px]">
              <button onClick={() => setConfirmDelete(false)} disabled={submitting}
                className="flex-1 py-[11px] border-none rounded-[12px] font-bold text-[14px] cursor-pointer"
                style={{ background: '#f6eef1', color: '#9b8aa3' }}>
                Cancel
              </button>
              <button onClick={handleDelete} disabled={submitting}
                className="flex-1 py-[11px] border-none rounded-[12px] font-bold text-[14px] cursor-pointer"
                style={{ background: '#FF4F6B', color: '#fff', opacity: submitting ? 0.6 : 1 }}>
